...
class TestPositive:
    ...
    def test_step9(self, start_time):
        # test9
        res = []
        res.append(checkout("echo '{}' | sudo -S dpkg -r" " {}".format(data["pkgname"]), "Удаляется"))
        res.append(checkout("echo '{}' | sudo -S {}".format(data["pkgname"]), "Status: deinstall ok"))
        assert all(res), "test10 FAIL"


'''
pytest test_01.py --junitxml=report.xml
pip install pytest-html
pytest test_01.py --html=report.html
pip install pytest-html-reporter
pytest test_01.py --html-report=report2.html
'''
